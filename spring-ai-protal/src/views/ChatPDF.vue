<template>
  <div class="chat-pdf" :class="{ 'dark': isDark }">
      ...
      <div class="sidebar">
        ...
        <div class="history-list">
          ...
          >
            ...
            <button 
              class="delete-btn" 
              @click.stop="showDeleteConfirm(chat.id, chat.title)"
              :title="'删除对话'"
            >
    ...
    <!-- 删除确认弹窗 -->
    <div v-if="showDeleteDialog" class="delete-dialog-overlay" @click="hideDeleteConfirm">
      <div class="delete-dialog" @click.stop>
        <div class="dialog-header">
          <h3>确认删除</h3>
        </div>
        <div class="dialog-content">
          <p>确定要删除对话 "{{ deleteTarget.title }}" 吗？</p>
          <p class="warning-text">此操作不可撤销，将删除所有相关的聊天记录和文档。</p>
        </div>
...
// 显示删除确认弹窗
const showDeleteConfirm = (chatId, title) => {
  deleteTarget.value = { id: chatId, title: title || 'PDF对话' }
...
const hideDeleteConfirm = () => {
  ...
  deleteTarget.value = { id: null, title: '' }
...
const confirmDelete = async () => {
  ...
  try {
    // 调用删除API
    await chatAPI.deletePdfChat(deleteTarget.value.id)
    
    // 从历史记录中移除
    chatHistory.value = chatHistory.value.filter(chat => chat.id !== deleteTarget.value.id)
    ...
    if (chatHistory.value.length === 0) {
      startNewChat()
    } else if (currentChatId.value === deleteTarget.value.id) {
      // 如果删除的是当前对话，加载第一个可用对话
...
const loadChat = async (chatId) => {
  ...
  try {
    ...
    if (chatIndex !== -1) {
      chatHistory.value[chatIndex].title = `PDF对话: ${filename.slice(0, 20)}${filename.length > 20 ? '...' : ''}`
    }
...
const handleDrop = async (event) => {
  ...
  try {
    ...
    currentPdfName.value = file.name  // 使用原始文件名
    pdfFile.value = file
