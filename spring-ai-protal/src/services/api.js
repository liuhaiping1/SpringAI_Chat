...
export const chatAPI = {
  ...
  // 删除PDF对话
  async deletePdfChat(chatId) {
    try {
      const response = await fetch(`${BASE_URL}/ai/pdf/pdf/${chatId}`, {
        method: 'DELETE'
      })
  ...
  async deleteChatHistory(type, chatId) {
    try {
      const response = await fetch(`${BASE_URL}/ai/${type}/${chatId}`, {
        method: 'DELETE'
      })
